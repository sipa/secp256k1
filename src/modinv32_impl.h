 ...
 * 2019.04.13 version.
 *
 * See the modinv64_impl.h file for an explanation of the algorithm and its implementation. This
 * file is an implementation for N=30 instead of N=62, without the need for wide 64-bit
 * multiplication, and with small variations in the optimizations.
 */
 ...
 * Return: final eta
 *
 * Implements the divsteps_n_matrix function from the explanation in modinv64_impl.h.
 */
 ...
 * Return: final eta
 *
 * Implements the divsteps_n_matrix_var function from the explanation in modinv64_impl.h.
 */
static uint32_t secp256k1_modinv32_divsteps_30_var(uint32_t eta, uint32_t f0, uint32_t g0, secp256k1_modinv32_trans2x2 *t) {
    ...
    for (;;) {
        ...
        /* m is a mask for the bottom min(limit, 8) bits (out table only supports 8 bits). */
 ...
 * (-2^30,2^30).
 *
 * This implements the update_de function from the explanation in modinv64_impl.h
 */
...
static void secp256k1_modinv32(secp256k1_modinv32_signed30 *x, const secp256k1_modinv32_modinfo *modinfo) {
    ...
    int i;
    uint32_t eta = -(uint32_t)1; /* Use the variant where eta = -(delta+1/2); delta is initially 1/2. */
...
static void secp256k1_modinv32_var(secp256k1_modinv32_signed30 *x, const secp256k1_modinv32_modinfo *modinfo) {
    ...
    secp256k1_modinv32_signed30 g = *x;
    int i, j, len = 9;
    uint32_t eta = -(uint32_t)1; /* eta = -delta; delta is initially 1 (faster for the variable-time code) */
    int32_t cond, fn, gn;

    /* Do up to 25 iterations of 30 divsteps each = 750 divsteps, or until g=0 (whichever comes first). */
    for (i = 0; i < 25; ++i) {
        /* Compute transition matrix and new eta after 30 divsteps. */
        secp256k1_modinv32_trans2x2 t;
